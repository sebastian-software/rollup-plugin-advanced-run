...
export async function getExitMap(basePath = "") {
  const exitMapEntries = await Promise.all(
    Object.entries(exitMap).reduce((prev: any[], current) => {
      const [key, value] = current;

      prev.push(
        // eslint-disable-next-line promise/prefer-await-to-then
        (value as Promise<number>).then(result => {
          return [key, result];
        })
      );
  ...
  return exitMapEntries.reduce((prev, current) => {
    ...
    prev[relative(basePath, key)] = value;
...
export function advancedRun(options: rollupOptions = {}) {
  ...
  return {
    ...
    generateBundle(outputOptions, bundle, isWrite) {
      if (!isWrite) {
        this.error(`rollup-plugin-advanced-run currently only works with bundles that are written to disk`);
      }
